...
| `GameBoard` (letture con token) | `catalog`, `completionsOf`, `participants`, `feed`, `profile`, `likers`, `features` (funzioni accese, ADR 0014), `permissions` (cosa può creare questa sessione, ADR 0018), `onChange(tabella)` |
...
| `Challenges` (ADR 0020) | `list` (con la sessione di giocatore o admin), `create` (restituisce l'id), `update` (`ChallengeEdit`: `extendMinutes` fa ripartire il tempo da adesso), `end` (termina subito), `remove` (autore o admin); `complete` e `undo` solo con `PlayerSession`: l'admin crea e gestisce ma non partecipa |
